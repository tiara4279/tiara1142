...
- 지급준비금 기본 시리즈는 `RESBALNS`이며, 후보 시리즈 폴백 로직으로 오래된 시리즈(`WRESBAL`)가 선택되지 않도록 신선도 검사를 함께 적용합니다.


## 변경 이력 메모

- main 기준 충돌 해결 시, 지급준비금 기준 시리즈(`RESBALNS`) 및 TGA 단위(백만→십억 달러) 보정 로직을 우선 유지합니다.
